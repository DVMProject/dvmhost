...
bool PatchStatusRegistry::publish(json::object& request, json::object& response, std::string& errorMessage, bool* changed)
{
    ...
    if (request["patches"].get<json::array>().size() > MAX_PATCHES_PER_PEER) {
        errorMessage = "patches exceeded the maximum allowed count";
        return false;
    }
    ...
    if (request["peerName"].is<std::string>()) {
        ...
        if (incoming.peerName.length() > MAX_STRING_LENGTH) {
            errorMessage = "peerName exceeded the maximum allowed length";
            return false;
        }
    }
...
bool PatchStatusRegistry::parsePatch(json::object& obj, PatchRecord& patch, std::string& errorMessage) const
{
    if (obj["patchId"].is<std::string>()) {
        ...
        if (patch.patchId.length() > MAX_STRING_LENGTH) {
            errorMessage = "patchId exceeded the maximum allowed length";
            return false;
        }
    }
    ...
    if (obj["members"].get<json::array>().size() > MAX_MEMBERS_PER_PATCH) {
        errorMessage = "patch members exceeded the maximum allowed count";
        return false;
    }
...
bool PatchStatusRegistry::parseMember(json::object& obj, PatchMember& member, std::string& errorMessage) const
{
    if (obj["system"].is<std::string>()) {
        ...
        if (member.system.length() > MAX_STRING_LENGTH) {
            errorMessage = "patch member system exceeded the maximum allowed length";
            return false;
        }
    }

    if (obj["mode"].is<std::string>()) {
        if (obj["mode"].get<std::string>().length() > MAX_STRING_LENGTH) {
            errorMessage = "patch member mode exceeded the maximum allowed length";
            return false;
        }
