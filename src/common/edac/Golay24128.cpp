...
#define GENPOL          0x00000c75   /* generator polynomial, g(x) */
...
void Golay24128::decode24128(uint8_t* data, const uint8_t* raw, uint32_t msglen)
{
    ...
    uint32_t r0, r1, r2, r3, r4, r5; // six 8-bit bytes
    uint32_t v0, v1;                 // two 24-bit encoded symbols
    uint32_t m0_hat, m1_hat;         // two 12-bit decoded symbols

    // determine remainder of input length / 3
    uint32_t r = msglen % 3;

    for (i = 0; i < msglen - r; i += 3) {
        ...
        data[i + 1] = ((m0_hat << 4) & 0xf0) | ((m1_hat >> 8) & 0x0f);
        data[i + 2] = ((m1_hat) & 0xff);

        j += 6;
    }

    // if input length isn't divisible by 3, decode last 1 or two bytes
    for (i = msglen - r; i < msglen; i++) {
...
{
    uint32_t j = 0;
    uint32_t s0, s1, s2;            // three 8-bit symbols
    uint32_t m0, m1;                // two 12-bit symbols (uncoded)
    uint32_t v0, v1;                // two 24-bit symbols (encoded)

    // determine remainder of input length / 3
    uint32_t r = msglen % 3;

    for (uint32_t i = 0; i < msglen - r; i += 3) {
    ...
    for (uint32_t i = msglen - r; i < msglen; i++) {
