...
TEST_CASE("AES", "[AES256 MAC CMAC Test]") {
    SECTION("P25_MAC_CMAC_Crypto_Test") {
        ...
        case KMM_MAC::DES_MAC:
            {
                ...
                ::LogInfoEx("T", "P25_MAC_CMAC_Crypto_Test, macAlgId = $%02X, macKId = $%04X", macAlgId, macKId);
                Utils::dump(2U, "MAC", mac, macLength);
            }
            break;
        ...
        for (uint32_t i = 0; i < 32U; i++) {
            if (macKey[i] != expectedCMAC[i]) {
                ::LogDebug("T", "P25_MAC_CMAC_Crypto_Test, INVALID AT IDX %d\n", i);
                failed = true;
            }
        }
